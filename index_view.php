...
<head>
	...
	<!-- Current Page CSS -->
	<link rel="stylesheet"
		href="<?php echo base_url(); ?>/spms/porto/assets/vendor/circle-flip-slideshow/css/component.css">
...
<div class="text-center my-5">
	<div class="col-md-10 mx-md-auto">
		<h1 class="word-rotator slide font-weight-bold text-8 mb-3 appear-animation"
			...
			<span style="font-size: 35px; margin-bottom:10px; margin-left:10px;">St. Patrick's Medical Systems
				(SPMS)</span><br>
			<span class="word-rotator-words" style="display: inline-block;">
				<b style="display: none; color:#e11910;">NURTURING</b>
				<b style="display: none; color:#e11910;">GROWTH</b>
				<b style="display: none; color:#e11910;">SOLUTION</b>
			</span>
		...
		<div class="container">
			...
			<div class="row mt-5 mb-5 pt-3 pb-3">
				<div class="col-md-8">
					...
					<iframe style="width:100%; height:80%; margin-bottom:100px;" src=""
						data-src="https://www.youtube.com/embed/videoseries?list=PLEFozZkV8lnPyX6MkwMkjQnaP_NrJtjTG"
				...
				<!-- Recent post -->
				<div class="col-md-4">
					<h3 class="text-color-secondary text-capitalize font-weight-bold text-5 m-0 mb-3">Recent
						<span>Posts</span> </h3>
					<?php
					$this->db->order_by('blog_added_date', 'desc');
					$query = $this->db->get('tbl_blog', 6);
					foreach ($query->result() as $row) {
						$date = strtotime($row->blog_added_date);
						$new_date1 = date('dS M Y', $date);
						?>
						<a href="#" class="text-color-default text-uppercase text-1 mb-0 d-block text-decoration-none"
							style="color: black;">
							<span style="color: red;"><?php echo $new_date1; ?></span>
							<span class="opacity-3 d-inline-block px-2">|</span>
							<?php echo $row->blog_added_by; ?>
						</a>
						<a href="<?php echo base_url() . 'blog/single/' . $row->blog_id . '/' . url_title($row->blog_title, 'dash', true); ?>"
							...
							<?php echo ucwords(strtolower($row->blog_title)); ?>
						</a>
					<?php } ?>
					<a href="<?php echo base_url(); ?>blog" class="button button-small button-border button-rounded">See
						more</a>
				</div>
			</div>
		</div>
...
<div class="container">
	...
	<div class="row">
		...
		$query = $this->db->get('tbl_prod_items', 9);
		foreach ($query->result() as $row) {
							...
							foreach ($catsubquery->result() as $subr) {
								...
								if ($qimg->result() != null) {
									foreach ($qimg->result() as $row1) {
										?>
										<img src="<?php echo base_url() . 'assets/images/products/thumb/' . $row1->prod_img_raw . '_thumb' . $row1->prod_img_ext; ?>"
											alt="<?php echo $row->p_item_name; ?> thumbnail">
										<?php
									}
								} else {
									?>
									<img src="<?php echo base_url(); ?>assets/images/default/defaultx54.jpg"
										alt="SPMS thumbnail default">
									<?php
								}
								?>
					</div>
						...
						<div class="entry-title">
							...
							<h5>
								...
								if ($catquery->result() != null) {
									?>
									<a
										href="<?php echo base_url(); ?>products/<?php echo url_title($row->p_item_category, "dash", TRUE); ?>/<?php echo url_title($row->p_item_name, "dash", TRUE); ?>">
										<?php
								} else {
									...
									foreach ($catsubquery->result() as $subr) {
										...
										foreach ($catsub->result() as $catr) {
											?>
												<a
													href="<?php echo base_url(); ?>products/<?php echo url_title($catr->p_cat_name, "dash", TRUE); ?>/<?php echo url_title($subr->p_s_cat_name, "dash", TRUE); ?>/<?php echo url_title($row->p_item_name, "dash", TRUE); ?>">
													<?php
										}
									}
								} ?>
	...
	<div class="text-center">
		<a href="<?php echo base_url(); ?>products" class="button button-small button-border button-rounded">See
			more</a>
	</div>
</div>
...
<div class="container">
	<div class="row pt-3">
		<div class="col">
			<h2 class="font-weight-semibold mb-0">Visit us:
				<span style="color: gray;">3/F East Wing St.Patrick's Square Bldg, 566 Shaw Blvd, Mandaluyong,
					1552</span>
			</h2>
		</div>
	</div>
	<div class="row">
		<div class="col-md-4">
			<span class="thumb-info-caption px-4 pb-3">
				<span class="thumb-info-caption-text p-xl">
					<h3>Call us today at <span style="background-color: transparent; color: black;"
							onmouseover="this.style.backgroundColor='white';"
							onmouseout="this.style.backgroundColor='transparent';">(02) 8533 8814</span> or Email us at
						<span style="background-color: transparent; color:black;"
							onmouseover="this.style.backgroundColor='white';"
							onmouseout="this.style.backgroundColor='transparent';">spmsphilippines@gmail.com</span></h3>
				</span>
			</span>
			</span>
			</a>
		...
		<div class="col-md-4">
			<span class="thumb-info-caption px-4 pb-3">
				<span class="thumb-info-caption-text p-xl">
					<h3><span style="color: black;">Enabling a successful healthcare practice through better customer
							solutions and employee excellence.</span>
				</span>
			</span>
			</span>
			</a>
		...
		<div class="col-md-4">
			<span class="thumb-info-caption px-4 pb-3">
				<span class="thumb-info-caption-text p-xl" style="margin-left: 50px;">
					<a href="<?php echo base_url(); ?>contact"
						class="button  button-xlarge button-border button-rounded" style="margin-top: 25px;">INQUIRE</a>
				</span>
			</span>
			</span>
			</a>
...
<a href="#" id="fixed1" data-toggle="modal" data-target="#subscribeModal" class="button button-small button-circle"><i
		class="icon-gift"></i> FREE gift!</a>
				...
				<div class="modal-body">
					<div class="form-widget">
						...
						<form class="nobottommargin" id="template-contactform" name="template-contactform"
							action="<?php echo base_url(); ?>inquiry/send_gift" method="post">

							<div class="form-process"></div>

							<div class="col_half">
								<label for="template-contactform-email-address">Email Address*</label>
								<input type="email" id="template-contactform-email-address"
									name="template-contactform-email-address" value=""
									class="sm-form-control required" />
							</div>
							<div class="col_half col_last">
								<label for="template-contactform-email-contact-number">Contact/Viber #*</label>
								<input type="text" id="template-contactform-number" name="template-contactform-number"
									value="" class="sm-form-control required" />
							</div>
							<div class="clear"></div>
							<div class="col_half">
								<label for="template-contactform-first-name">First Name*</label>
								<input type="text" id="template-contactform-first-name"
									name="template-contactform-first-name" value="" class="required sm-form-control" />
							</div>

							<div class="col_half col_last">
								<label for="template-contactform-phone">Last Name*</label>
								<input type="text" id="template-contactform-last-name"
									name="template-contactform-last-name" value="" class="sm-form-control required" />
							</div>
							<div class="clear"></div>
							<div class="col_full">
								<label for="template-contactform-area">Area / City*</label>
								<input list="loc" type="text" id="template-contactform-area"
									name="template-contactform-area" value="" class="sm-form-control required" />
								<datalist id="loc">
							...
							<div class="col_half">
								...
								<input type="number" id="template-contactform-prc-no" name="template-contactform-prc-no"
									value="" class="required sm-form-control" />
							</div>

							<div class="col_half col_last">
								<label for="template-contactform-birthday">Birthday*</label>
								<input type="date" id="template-contactform-birthday"
									name="template-contactform-birthday" value="<?php echo date('Y-m-d'); ?>"
									class="sm-form-control required" />
							</div>
							<div class="clear"></div>
							...
							<div class="col_half">
								...
								<select name="template-contactform-type" id="template-contactform-type"
									class="sm-form-control required">
							...
							<div class="col_half col_last">
								...
								<input type="text" id="template-contactform-know-us" name="template-contactform-know-us"
									value="" class="sm-form-control required" />
							</div>
							...
							<div class="col_full">
								...
								<select name="template-contactform-package" id="template-contactform-package"
									class="sm-form-control required">
									...
									<option>pack of 1 each (Supreme, Implant ortho, Compact tuff, Special care, Tepe
										mini, ne pack assorted Interdental brush)</option>
							...
							<div class="col_full">
								...
								<center>
									<div class="g-recaptcha" data-sitekey="6LcqPXAUAAAAAMradOQi7C3TOeBhNnEQG-Hrb1IF">
									</div>
								</center>
							</div>
							...
							<div class="col_full text-center">
								<button name="submit" type="submit" id="submit-button" value="Submit"
									class="button button-3d nomargin">Subscribe and get gift</button>
							</div>
